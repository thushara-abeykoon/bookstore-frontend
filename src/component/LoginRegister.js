...
const LoginRegister = ({component, method}) => {
    ...
    return (
        <div className="md:float-right md:w-5/6 md:h-screen flex items-center justify-center">
            <div className="m-auto text-center max-md:w-full max-md:px-16 bg-white px-28 py-20 shadow-xl rounded-lg flex flex-col gap-20 w-1/2">
                ...
                <form onSubmit={e=>{
                    e.preventDefault();
                    method({username:userNameRef.current.value, password:passwordRef.current.value});
                }} className="flex flex-col gap-16">
                    <div className="flex flex-col gap-10">
                        <input ref={userNameRef} type="text" placeholder="Username" className="text-lg p-2 border-b-2 border-black outline-none bg-transparent" />
                        <input ref={passwordRef} type="password" placeholder="Password" className="text-lg p-2 border-b-2 border-black outline-none bg-transparent" />
                    </div>
